...
export function Header() {
  ...
  return <header className="sticky top-0 z-20 border-b border-ink/10 bg-paper/95 backdrop-blur-sm"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 sm:px-10">
    ...
    <nav className="hidden items-center gap-7 text-[11px] uppercase tracking-[0.18em] text-ink/65 md:flex">
      <Link to="/explore" className="hover:text-oxblood">Explore</Link><Link to="/curator" className="hover:text-oxblood">Curator</Link><Link to="/museum" className="hover:text-oxblood">Museum</Link><Link to="/auctions" className="hover:text-oxblood">Auctions</Link><Link to="/artists" className="hover:text-oxblood">Artists</Link><span>Exhibitions</span>
    </nav>
    <div className="flex items-center gap-4 text-sm">{isAuthenticated?<><NotificationBell /><Link to="/studio" className="hidden text-ink/65 hover:text-oxblood sm:block">Studio</Link><Link to="/studio/media" className="hidden text-ink/65 hover:text-oxblood lg:block">Images</Link><span className="hidden text-ink/60 xl:inline">{user?.displayName}</span><button onClick={()=>{logout();navigate('/')}} className="btn-outline">Sign out</button></>:<><Link to="/login" className="text-ink/70 hover:text-oxblood">Sign in</Link><Link to="/register" className="btn-outline">Join ARTVERSE</Link></>}</div>
  </div></header>
}
