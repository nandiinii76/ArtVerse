...
public class MarketplaceService {
    ...
    public MarketplaceDtos.OrderResponse createAuctionSettlementOrder(Auction auction) {
        ...
        Artwork artwork = artwork(auction.getArtworkId());
        Order order = new Order(); order.setBuyerId(auction.getWinnerId()); order.setSellerId(auction.getSellerId()); order.setAuctionId(auction.getId()); order.setArtworkId(auction.getArtworkId()); order.setAmount(auction.getCurrentPrice()); order.setCurrency(auction.getCurrency()); order.setStatus(OrderStatus.PENDING);
        Order saved = orders.save(order);
        notificationEvents.auctionPaymentDue(auction.getWinnerId(), artwork.getTitle(), auction.getCurrentPrice(), auction.getCurrency());
        return MarketplaceDtos.OrderResponse.from(saved);
    }
    ...
    public MarketplaceDtos.OrderResponse completeAuctionPayment(UUID orderId, PaymentRequest request, User buyer) {
        ...
        order.setStatus(OrderStatus.PAID); order.setPaymentReference(request.paymentReference().trim());
        ArtworkOwnership transfer = new ArtworkOwnership(); transfer.setArtworkId(order.getArtworkId()); transfer.setOwnerId(buyer.getId()); transfer.setTransactionId(order.getId()); ownership.save(transfer);
        Artwork artwork = artwork(order.getArtworkId());
        notificationEvents.auctionPaymentReceived(order.getSellerId(), buyer.getId(), artwork.getTitle());
    ...
    public MarketplaceDtos.OrderResponse completePayment(UUID orderId, PaymentRequest request, User buyer) {
        ...
        order.setStatus(OrderStatus.PAID); order.setPaymentReference(request.paymentReference().trim()); listing.setStatus(ListingStatus.SOLD);
        ArtworkOwnership transfer = new ArtworkOwnership(); transfer.setArtworkId(order.getArtworkId()); transfer.setOwnerId(buyer.getId()); transfer.setTransactionId(order.getId()); ownership.save(transfer);
        Order saved = orders.save(order); notificationEvents.marketplaceOrderPaid(listing.getSellerId(), buyer.getId(), artwork(order.getArtworkId()).getTitle()); return MarketplaceDtos.OrderResponse.from(saved);
    }
