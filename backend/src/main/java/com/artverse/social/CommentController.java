...
public class CommentController {
    ...
    public CommentController(CommentRepository repository, SocialNotificationService notifications) {
    ...
    public Response create(@PathVariable UUID artworkId, @Valid @RequestBody CreateRequest request, @AuthenticationPrincipal User user) {
        ...
        Comment saved = repository.save(c);
        notifications.comment(artworkId, user);
        return Response.from(saved);
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Void> delete(@PathVariable UUID id, @AuthenticationPrincipal User user) {
        Comment c = repository.findById(id).orElseThrow(() -> new org.springframework.web.server.ResponseStatusException(
                org.springframework.http.HttpStatus.NOT_FOUND, "Comment not found"));
        if (!c.getUserId().equals(user.getId())) throw new org.springframework.web.server.ResponseStatusException(
                org.springframework.http.HttpStatus.FORBIDDEN, "Only the author can delete this comment");
        repository.delete(c);
